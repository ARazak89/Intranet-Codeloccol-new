...
export default function Dashboard() {
  ...
  const [expandedProjectEvaluations, setExpandedProjectEvaluations] = useState({}); // Nouveau: État pour gérer les détails des évaluations de projet déroulées pour le staff/admin
  ...
  const [showDeleteSlotModal, setShowDeleteSlotModal] = useState({ show: false, slotId: null, slotStartTime: null }); // Nouvel état pour le modal de suppression de slot
  ...
  const [currentEvaluationToReassign, setCurrentEvaluationToReassign] = useState(null); // L'évaluation sélectionnée pour la réassignation
  const [reassignSlotDate, setReassignSlotDate] = useState(''); // Date sélectionnée pour les nouveaux slots
  const [availableSlotsForReassign, setAvailableSlotsForReassign] = useState([]); // Slots disponibles pour la réassignation
...
//  };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (userData.role === 'staff' || userData.role === 'admin') {
        ...
        const allPendingEvalsRes = await fetch(`${API}/evaluations/all-for-staff`, { headers: { Authorization: `Bearer ${token}` } }); // Mise à jour de l'URL
  ...
  const handleDeleteSlot = async () => {
    ...
    try {
      const res = await fetch(`${API}/availability/${showDeleteSlotModal.slotId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      ...
      if (res.ok) {
        ...
        setShowDeleteSlotModal({ show: false, slotId: null, slotStartTime: null }); // Fermer la modale
  ...
  const handleSubmitHackathonProject = async (e) => {
    ...
    try {
      const res = await fetch(`${API}/hackathons/${currentHackathonToSubmit._id}/submit-project`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="container-fluid mt-4 pt-5 px-4">
                    ...
                    <div key={slot._id} className="list-group-item d-flex justify-content-between align-items-center flex-wrap py-3 list-group-item-action">
                      <div>
                        <h5 className="mb-1 text-primary d-flex align-items-center">
                          ...
                          <span>
                            {new Date(slot.startTime).toLocaleDateString('fr-FR', { timeZone: 'UTC', year: 'numeric', month: 'long', day: 'numeric' })} de {new Date(slot.startTime).toLocaleTimeString('fr-FR', { timeZone: 'UTC', hour: '2-digit', minute: '2-digit' })} à {new Date(slot.endTime).toLocaleTimeString('fr-FR', { timeZone: 'UTC', hour: '2-digit', minute: '2-digit' })}
                          </span>
                        </h5>
                        <div className="d-flex align-items-center">
                          {slot.isBooked ? (
                            <span className="badge bg-success rounded-pill me-2"><i className="bi bi-person-check-fill me-1"></i> Réservé</span>
                          ) : (
                            <button
                              className="btn btn-sm btn-outline-danger d-flex align-items-center me-2"
                              onClick={() => setShowDeleteSlotModal({ show: true, slotId: slot._id, slotStartTime: new Date(slot.startTime) })}
                            >
                          ...
                          <button
                            ...
                            onClick={() => setExpandedSlots(prev => ({ ...prev, [slot._id]: !prev[slot._id] }))}
                      ...
                      {expandedSlots[slot._id] && (
                        <div id={`slot-details-${slot._id}`} className="collapse show mt-3 ps-3 border-start border-primary border-2">
                          {slot.isBooked ? (
                            <>
                              <p className="mb-1"><i className="bi bi-person me-2"></i> Réservé par: <strong>{slot.bookedByStudent ? slot.bookedByStudent.name : '[Utilisateur inconnu]'}</strong></p>
                              <p className="mb-1"><i className="bi bi-journal-text me-2"></i> Pour le projet: <strong>{slot.bookedForProject ? slot.bookedForProject.title : '[Projet inconnu]'}</strong></p>
                            </>
                          ) : (
                            <p className="mb-1 text-muted"><i className="bi bi-check-circle me-2"></i> Ce slot est actuellement disponible.</p>
                          )}
                          <p className="mb-1"><i className="bi bi-info-circle me-2"></i> Créé le: {new Date(slot.createdAt).toLocaleString()}</p>
                        </div>
                      )}
                    </div>
                  ))}
                    ...
                    <div key={project.assignmentId} className="card mb-3 shadow-sm border-info">
                      <div className="card-body">
                        <div className="d-flex justify-content-between align-items-center flex-wrap">
                          ...
                          <button
                            className="btn btn-sm btn-outline-secondary"
                            onClick={() => setExpandedFeedback(prev => ({ ...prev, [project.assignmentId]: !prev[project.assignmentId] }))}
                        ...
                        {expandedFeedback[project.assignmentId] && (
                          <div id={`project-details-${project.assignmentId}`} className="collapse show mt-3">
                  ...
                  {me.role === 'apprenant' ? (
                    ...
                    upcomingEvaluations.map((evaluation) => {
                      ...
                      const buttonText = isEvaluationActive ? 'Évaluer le projet' :
                                         now < evaluationStartTime ? `Actif à ${evaluationStartTime.getUTCHours().toString().padStart(2, '0')}:${evaluationStartTime.getUTCMinutes().toString().padStart(2, '0')}` :
                                         'Évaluation expirée';
                        ...
                        <li key={evaluation._id} className="list-group-item">
                          <div className="d-flex justify-content-between align-items-center flex-wrap py-3">
                            <div className="d-flex flex-column">
                              <h5 className="mb-1 text-info d-flex align-items-center">
                                <i className="bi bi-calendar-check me-2"></i> Projet: {evaluation.project.title}
                              </h5>
                              <small className="text-muted d-flex align-items-center mt-1">
                                <i className="bi bi-clock me-1"></i> Date: {evaluationStartTime.toLocaleDateString()} de {evaluationStartTime.getUTCHours().toString().padStart(2, '0')}:{evaluationStartTime.getUTCMinutes().toString().padStart(2, '0')} à {evaluationEndTime.getUTCHours().toString().padStart(2, '0')}:{evaluationEndTime.getUTCMinutes().toString().padStart(2, '0')}
                              </small>
                            </div>
                            <button
                              className="btn btn-sm btn-outline-secondary"
                              onClick={() => setExpandedFeedback(prev => ({ ...prev, [evaluation._id]: !prev[evaluation._id] }))}
                              aria-expanded={!!expandedFeedback[evaluation._id]}
                              aria-controls={`upcoming-eval-details-${evaluation._id}`}
                            >
                              <i className={`bi bi-chevron-${expandedFeedback[evaluation._id] ? 'up' : 'down'}`}></i>
                            </button>
                          </div>
                          {expandedFeedback[evaluation._id] && (
                            <div id={`upcoming-eval-details-${evaluation._id}`} className="collapse show mt-3 ps-3 border-start border-info border-2">
                      ...
                      Object.values(allPendingEvaluationsForStaff.filter(evaluation => {
                        ...
                        <li key={projectGroup.project._id} className="list-group-item d-flex flex-column align-items-start flex-wrap mb-3 py-3">
                          <div className="d-flex justify-content-between align-items-center w-100">
                            <div className="d-flex flex-column">
                              <h5 className="mb-1"><i className="bi bi-journals me-2"></i> Projet: {projectGroup.project.title} (Soumis par: {projectGroup.evaluations[0]?.studentName || 'N/A'})</h5>
                            ...
                            >
                              <i
                                className={`bi bi-chevron-${
                                  expandedProjectEvaluations[projectGroup.project._id] ? 'up' : 'down'
                                }`}
                              ></i>
                            </button>
                          </div>
                          {expandedProjectEvaluations[projectGroup.project._id] && (
                            <div id={`project-evals-details-${projectGroup.project._id}`} className="collapse show mt-3 w-100 ps-3 border-start border-info border-2">
                                  ...
                                  return (
                                    <li key={evalItem._id} className="list-group-item d-flex justify-content-between align-items-center flex-wrap">
                                      ...
                                      <div>
                                        ...
                                        { (evalItem.status === 'rejected' || (evalItem.status === 'pending' && now > gracePeriodEnd)) && (
                                          <button
                    ...
                    <li key={project._id} className="list-group-item d-flex justify-content-between align-items-center flex-wrap py-3">
                      ...
                      <div className="d-flex flex-column flex-md-row mt-2 mt-md-0">
                        <button
                          ...
                          onClick={() => token && handleFinalStaffReview(project.projectId, project.assignmentId, 'approved')}
                        >
                          <i className="bi bi-check-circle me-1"></i> Approuver
                        </button>
                        <button
                          className="btn btn-sm btn-danger mt-2 mt-md-0"
                          onClick={() => token && handleFinalStaffReview(project.projectId, project.assignmentId, 'rejected')}
                        >
                    ...
                    <div key={evaluation._id} className="list-group-item">
                      <div className="d-flex justify-content-between align-items-center flex-wrap">
                        <h5 className="mb-2 d-flex align-items-center">
                          <i className="bi bi-journal-check me-2"></i> Projet: {evaluation.project.title}
                          <span className={`badge bg-${evaluation.status === 'accepted' ? 'success' : evaluation.status === 'pending' ? 'info' : 'danger'} ms-2`}>
                            {evaluation.status === 'accepted' ? 'Accepté' : evaluation.status === 'pending' ? 'En attente' : 'Rejeté'}
                          </span>
                        </h5>
                        {evaluation.feedback && (
                          <button
                      ...
                      {expandedFeedback[evaluation._id] && (
                        <div id={`feedback-details-${evaluation._id}`} className="collapse show mt-3">
          ...
          <div className="col-12">
            <div className="card shadow-sm">
              <div className="card-header bg-gradient bg-info text-white d-flex align-items-center">
                ...
                <h2 className="h5 mb-0">Mes Notifications</h2>
              </div>
              <div className="card-body">
                <ul className="list-group list-group-flush">
                  {notifications.map((notification) => (
                    <li
                      ...
                      className="list-group-item d-flex justify-content-between align-items-center flex-wrap py-3 list-group-item-action"
                      onClick={() => handleOpenNotificationModal(notification)}
                      style={{ cursor: 'pointer' }}
                    >
                      <div>
                        <h5 className="mb-1 text-info"><i className="bi bi-chat-dots me-2"></i> {notification.message.substring(0, 100)}{notification.message.length > 100 ? '...' : ''}</h5>
                        <small className="text-muted d-flex align-items-center mt-1"><i className="bi bi-clock me-1"></i> {new Date(notification.createdAt).toLocaleString()}</small>
                      </div>
                      {!notification.read && (
                        <span className="badge bg-warning text-dark rounded-pill"><i className="bi bi-eye me-1"></i> Nouvelle</span>
                      )}
                    </li>
                  ))}
                </ul>
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              <div className="modal-header bg-danger text-white">
                <h5 className="modal-title"><i className="bi bi-exclamation-triangle me-2"></i> Confirmer la Suppression du Slot</h5>
                <button type="button" className="btn-close" onClick={() => setShowDeleteSlotModal({ show: false, slotId: null, slotStartTime: null })}></button>
              </div>
              <div className="modal-body">
                {error && <div className="alert alert-danger mb-3" role="alert">{error}</div>}
                <p>Êtes-vous sûr de vouloir supprimer le slot de disponibilité commençant le</p>
                <p><strong>{showDeleteSlotModal.slotStartTime?.toLocaleString()}</strong> ?</p>
                <p className="text-danger">Cette action est irréversible et ne peut être faite que si le slot n'est pas réservé.</p>
              </div>
              <div className="modal-footer">
                <button type="button" className="btn btn-secondary" onClick={() => setShowDeleteSlotModal({ show: false, slotId: null, slotStartTime: null })}>Annuler</button>
                <button type="button" className="btn btn-danger" onClick={handleDeleteSlot} disabled={isLoading}>
                  {isLoading ? (
                    <span className="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>
                  ) : (
      ...
      {showDeleteSlotModal.show && <div className="modal-backdrop fade show"></div>}
          ...
          <div className="modal-dialog modal-dialog-centered" role="document">
            <div className="modal-content">
              <div className="modal-header bg-warning text-dark">
                ...
                <button type="button" className="btn-close" aria-label="Close" onClick={() => setShowReassignModal(false)}></button>
              </div>
              <div className="modal-body">
                <p>Réassigner l'évaluation du projet:
                ...
                <div className="mb-3">
                  <label htmlFor="slotDate" className="form-label">Date des nouveaux slots</label>
                  <input
                    type="date"
                    id="slotDate"
                    className="form-control"
                    ...
                    {availableSlotsForReassign.map(slot => (
                      <div key={slot._id} className="form-check">
                        ...
                        <label className="form-check-label" htmlFor={`slot-${slot._id}`}>
                          {new Date(slot.startTime).toLocaleString()} - {new Date(slot.endTime).toLocaleString()} ({slot.evaluator.name})
                        </label>
                      </div>
                    ))}
                  </div>
              ...
              <div className="modal-footer">
                <button type="button" className="btn btn-secondary" onClick={() => setShowReassignModal(false)}>Annuler</button>
                <button
                  type="button"
