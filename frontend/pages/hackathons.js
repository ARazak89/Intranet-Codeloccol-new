...
export default function Hackathons() {
  ...
  const fetchHackathonsData = async () => {
    ...
    try {
      ...
      if (me && (me.role === 'staff' || me.role === 'admin')) {
        const learnersRes = await fetch(`${API}/hackathons/available-learners`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleConstituteTeams = async () => {
    ...
    try {
      const res = await fetch(`${API}/hackathons/${selectedHackathonForTeams._id}/constitute-teams`, {
        method: 'POST',
