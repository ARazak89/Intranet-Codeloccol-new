...
const HackathonList = ({ hackathons, me, onShowSubmitHackathonModal }) => {
  ...
  return (
    <div className="thm-shadow-s thm-bg mb-4">
        ...
        {hackathons.map((hackathon) => (
          <li key={hackathon._id} className="list-group-item">
            ...
            <small className="text-muted">
              Début: {new Date(hackathon.startDate).toLocaleDateString()} Fin:{" "}
              {new Date(hackathon.endDate).toLocaleDateString()}
            </small>
            {hackathon.teamSize && (
              <p className="mb-1 text-muted">
                Taille d'équipe: {hackathon.teamSize}
              </p>
            )}
            {me &&
              me.role === "apprenant" &&
              ...
              (() => {
                ...
                if (myTeam) {
                  ...
                  if (!myTeam.repoUrl && isSubmissionPeriodActive) {
                    // Supposons que myTeam.repoUrl indique si le projet est soumis
                    return (
                      <button
                        className="btn btn-sm btn-primary mt-2"
                        onClick={() =>
                          onShowSubmitHackathonModal(hackathon, myTeam)
                        }
                      >
                        <i className="bi bi-upload me-1"></i> Soumettre le
                        Projet de Hackathon
                      </button>
                    );
                    ...
                    return (
                      <span className="badge bg-success mt-2 d-flex align-items-center">
                        <i className="bi bi-check-circle me-1"></i> Projet
                        Soumis
                        <a
                          href={myTeam.repoUrl}
                          target="_blank"
                    ...
                    return (
                      <span className="badge bg-secondary mt-2">
                        <i className="bi bi-x-circle me-1"></i> Période de
                        soumission terminée
                      </span>
                    );
                  }
