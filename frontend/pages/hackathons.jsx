...
export default function Hackathons() {
  ...
  const loadHackathonTeams = async (hackathonId) => {
    ...
    try {
      const r = await fetch(`${API}/teams/hackathon/${hackathonId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleConstituteTeams = async () => {
    ...
    for (const team of currentTeams) {
      ...
      if (team.members.length > selectedHackathonForTeams.teamSize) {
        setError(
          `L'équipe ${team.name} dépasse la taille maximale autorisée de ${selectedHackathonForTeams.teamSize} membres.`
        );
        return;
      }
      ...
      const loadUserData = async () => {
        try {
          ...
          if (userData.role === "staff" || userData.role === "admin") {
            const learnersRes = await fetch(`${API}/users?role=apprenant`, {
              headers: { Authorization: `Bearer ${token}` },
            });
          ...
          <div className="modal-dialog modal-lg">
            <div className="modal-content">
              ...
              <div className="modal-body">
                ...
                {selectedHackathon.participants &&
                  selectedHackathon.participants.length > 0 && (
                    <div>
                      <p>
                        <strong>
                          Participants ({selectedHackathon.participants.length}
                          ):
                        </strong>
                      </p>
                ...
                {selectedHackathon.projects &&
                  selectedHackathon.projects.length > 0 && (
                    <div className="mt-3">
                      <p>
                        <strong>
            ...
            <div className="modal-content">
              ...
              <div className="modal-body thm-bg">
                <form onSubmit={handleCreateHackathon}>
                  ...
                  <div className="mb-3">
                    <label htmlFor="newHackathonEndDate" className="form-label">
                      Date de fin
                    </label>
                    <input
                      type="date"
