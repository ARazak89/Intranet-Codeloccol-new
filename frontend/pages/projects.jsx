...
function ProjectsPage() {
  ...
  const handleOpenSubmitProjectModal = async (project) => {
    ...
    try {
      ...
      const slotsRes = await fetch(
        `${API}/availability/available-for-project/${project.projectId}/${project.assignmentId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      if (slotsRes.ok) {
        ...
        setErrorPopupMessage(
          errorData.error || "Impossible de charger les créneaux disponibles."
        );
  ...
  const handleSubmitProject = async (e) => {
    ...
    if (
      ...
      let errorMessage =
        "Veuillez sélectionner exactement 2 créneaux d'évaluateurs différents.";
      if (!isRepoUrlOptional && !projectSubmissionRepoUrl) {
        errorMessage =
          "Veuillez fournir l'URL du dépôt GitHub et sélectionner exactement 2 créneaux d'évaluateurs différents.";
      }
    ...
    if (uniqueEvaluators.length !== 2) {
      setErrorPopupMessage(
        "Vous devez sélectionner exactement 2 créneaux, chacun d'un évaluateur différent."
      );
      setShowErrorPopup(true);
      setPopupType("error");
      setLoading(false);
                  ...
                  <form onSubmit={handleSubmitProject}>
                    ...
                    <div className="mb-4 p-3 bg-light border rounded">
                      <label className="form-label d-block mb-3">
                        ...
                        Sélectionner 2 Créneaux d'Évaluation (Obligatoire){" "}
                        <span className="text-danger">*</span>
                      </label>
                      <div className="alert alert-info py-2">
                        <i className="bi bi-info-circle me-2"></i>
                        <strong>Important :</strong> Vous devez sélectionner
                        exactement 2 créneaux d'évaluateurs différents pour que
                        votre projet soit évalué.
                      </div>
                      {availableSlots.length > 0 ? (
                              ...
                              <div className="form-check form-check-inline border rounded p-3 w-100 bg-white shadow-sm">
                                  ...
                                  onChange={(e) => {
                                    if (e.target.checked) {
                                      if (selectedSlotIds.length < 2) {
                                        ...
                                        setErrorPopupMessage(
                                          "Vous ne pouvez sélectionner que 2 créneaux maximum."
                                        );
                                        setShowErrorPopup(true);
                                ...
                                >
                                  <div className="d-flex flex-column align-items-start ms-2">
                                    ...
                                    <small className="text-info mt-1 d-flex align-items-center">
                                      <i className="bi bi-calendar-check me-1"></i>{" "}
                                      Créneau d'évaluation disponible
                                    </small>
                                  </div>
                                </label>
                              </div>
                        ...
                        <div className="alert alert-warning py-2">
                          ...
                          Aucun créneau d'évaluation disponible pour le moment.
                          Veuillez réessayer plus tard.
                        </div>
                      )}
                      <div className="form-text mt-3 text-dark">
                        <i className="bi bi-clipboard-check me-1"></i> Créneaux
                        sélectionnés: <strong>{selectedSlotIds.length}</strong>
                        /2
                        {selectedSlotIds.length === 2 && (
